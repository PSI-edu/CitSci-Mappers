...
If you are working on a system that has a publicly accessible URL, enter the following for you action code
```
...
exports.onContinuePostLogin = async (event, api) => {
    ...
    if (event.request.body.confirm === "yes") {
      ...
      axios.post(event.secrets.API_ENDPOINT_USER_NEW, 
        { email: event.user.email },
        { 
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${event.secrets.API_AUTH_SECRET}`
          }
        }
        )
        .then(response => {
          console.log('hit api', response.data);
        })
        .catch(error => { // Add error handling for your API call
            console.error('Error hitting user API:', error.message);
        });
